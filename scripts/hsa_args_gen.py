...
api_args_pattern = re.compile(re.escape(r"inline std::ostream& operator<< (std::ostream& out, const hsa_api_data_pair_t& data_pair) {"))
...
with open(api_str_hfile, "r") as input_file:
    ...
    with open("./src/hsa_args_str.cpp", "w") as cpp_file:
        ...
        while(1):
            if(case_pattern.search(next_line)):
                ...
                for i in range(4):
                    ...
                    next_line = input_file.readline()
                break
        api_args_end = ("  return hsa_api_string_pair_t(out.str(), ret.str());\n"
                        "}\n\n")
